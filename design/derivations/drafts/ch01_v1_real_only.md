...
|--------|---------|------------|
| $v$ | A computed or measured scalar value | §1.2 |
...
The results in this section apply independently to each error category. The proofs are stated for a generic error bound $\delta$, representing any of $\sigma_m$, $\delta_p$, or $\delta_a$. Let $x$, $y$ be computed values with error bounds $\delta_x$, $\delta_y$ respectively, so $|\tilde{x} - x| \leq \delta_x$ and $|\tilde{y} - y| \leq \delta_y$.
...
**Theorem 1.4.1** (Mean Value Theorem bound)**.** *Let $f: \mathbb{R} \to \mathbb{R}$ be continuously differentiable on an interval containing $[x - \delta, x + \delta]$. Then*

$$
|f(x + \epsilon) - f(x)| \leq \sup_{\xi \in [x-\delta, x+\delta]} |f'(\xi)| \cdot |\epsilon| \tag{1.9}
$$

*for all $|\epsilon| \leq \delta$.*

*Proof.* By the Mean Value Theorem, there exists $\xi$ between $x$ and $x + \epsilon$ such that $f(x + \epsilon) - f(x) = f'(\xi)\,\epsilon$. Since $|\epsilon| \leq \delta$, the point $\xi$ lies in $[x - \delta, x + \delta]$, so $|f'(\xi)| \leq \sup_{\xi' \in [x-\delta, x+\delta]} |f'(\xi')|$. The result follows. ∎

**Remark.** Theorem 1.4.1 applies to each error category independently. If $\mathcal{X} = (x, \sigma_m, \delta_p, \delta_a)$, then $\mathcal{F} = (f(x), \sigma_m^f, \delta_p^f, \delta_a^f)$ where each bound is computed from (1.9) with the corresponding $\delta$.

**Theorem 1.4.2** (Multivariate error propagation)**.** *Let $f: \mathbb{R}^n \to \mathbb{R}$ be continuously differentiable on an open set containing $\{\mathbf{x} + \boldsymbol{\epsilon} : |\epsilon_i| \leq \delta_i\}$. Then*
...
where $\boldsymbol{\epsilon}^{(i)}$ agrees with $\boldsymbol{\epsilon}$ in the first $i$ components and is zero in the remaining $n-i$ components (so $\boldsymbol{\epsilon}^{(0)} = \mathbf{0}$ and $\boldsymbol{\epsilon}^{(n)} = \boldsymbol{\epsilon}$). Each bracketed term changes only the $i$-th argument. By the univariate Mean Value Theorem applied to the $i$-th component:

$$
|f(\mathbf{x} + \boldsymbol{\epsilon}^{(i)}) - f(\mathbf{x} + \boldsymbol{\epsilon}^{(i-1)})| = \left|\frac{\partial f}{\partial x_i}(\boldsymbol{\xi}^{(i)})\right| \cdot |\epsilon_i|
$$

for some $\boldsymbol{\xi}^{(i)}$ on the line segment between $\mathbf{x} + \boldsymbol{\epsilon}^{(i-1)}$ and $\mathbf{x} + \boldsymbol{\epsilon}^{(i)}$, which lies within the box. Summing and bounding:
...
For vector-valued functions $\mathbf{f}: \mathbb{R}^n \to \mathbb{R}^m$, the bound applies component-wise; the Jacobian matrix $\partial f_j/\partial x_i$ governs the propagation. Extension to matrix operations, including the 13$\times$13 state matrix framework, is developed in Chapter 2.
...
*The bound is capped at $2$ (the diameter of the range of $\sin$).*
...
$$
|\sin(x + \epsilon) - \sin(x)| = \left|\cos(x)\,\epsilon - \frac{\sin(\xi)}{2}\,\epsilon^2\right| \leq |\cos(x)|\,|\epsilon| + \frac{|\sin(\xi)|}{2}\,\epsilon^2 \leq |\cos x|\,\delta + \frac{\delta^2}{2}
$$

since $|\sin(\xi)| \leq 1$ and $|\epsilon| \leq \delta$. The bound cannot exceed $|\sin(x + \epsilon) - \sin(x)| \leq |\sin(x+\epsilon)| + |\sin(x)| \leq 2$. ∎
...
*The bound is capped at $2$.*

*Proof.* Identical to Corollary 1.4.1, using the Taylor expansion of $\cos$ about $x$:

$$
\cos(x + \epsilon) = \cos(x) - \sin(x)\,\epsilon - \frac{\cos(\xi)}{2}\,\epsilon^2.
$$

Then $|\cos(x+\epsilon) - \cos(x)| \leq |\sin x|\,|\epsilon| + \frac{|\cos \xi|}{2}\,\epsilon^2 \leq |\sin x|\,\delta + \delta^2/2$, capped at $2$. ∎
...
**Corollary 1.4.8** (Absolute value bound)**.** *For any $x$ and $|\epsilon| \leq \delta$,*

$$
||x + \epsilon| - |x|| \leq \delta. \tag{1.18}
$$

*Proof.* By the reverse triangle inequality, $||x+\epsilon| - |x|| \leq |\epsilon| \leq \delta$. Equivalently, $|\cdot|$ is Lipschitz continuous with Lipschitz constant $1$. ∎
...
**Remark.** The choice of truncation index $N$ is governed by a caller-specified tolerance parameter, developed in Chapter 4. At the SGP4-standard tolerance, $N$ matches the original model's truncation point exactly — reproducing the legacy result. At tighter tolerances, additional terms are computed, with the remainder bound (Theorem 1.5.2 or 1.5.3) added to $\delta_p$. This is the Matched Pair Principle (Ch. 3) applied to series: the generalized routine accepts a tolerance that, at its standard value, reproduces the original model.

---

## §1.6 Propagation Through Iterative Convergence

**Assumption 1.6.1.** *Let $g: D \to D$ be a contraction mapping on a closed subset $D \subseteq \mathbb{R}$ with Lipschitz constant $L < 1$, i.e., $|g(x) - g(y)| \leq L|x - y|$ for all $x, y \in D$.*
...
**Theorem 1.9.1** (Reliable digits under univariate functions)**.** *Let $\mathcal{V}_1 = (v_1, \sigma_1, \delta_{p,1}, \delta_{a,1})$ with $v_1 \neq 0$, and let $f$ be a continuously differentiable function applied with the Mean Value Theorem propagation rule (Theorem 1.4.1). Define $\mathcal{V}_2 = (f(v_1), \sigma_2, \delta_{p,2}, \delta_{a,2})$. Then, provided $f(v_1) \neq 0$:*
