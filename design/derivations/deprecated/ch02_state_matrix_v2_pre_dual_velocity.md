...
The inverse operation — recovering $M$ from a known $3 \times 3$ rotation matrix $R$ — is needed when converting from legacy representations into the $SU(2)$ framework. The algorithm (Shepperd's method) selects the quaternion component with the largest magnitude to avoid numerical instability. The full development is in §2.9 (Theorem 2.9.1), after the adjoint bridge (§2.7) establishes the forward map $M \to R(M)$.
...
The dual $SU(2)$ element $\hat{M}$ encodes a rigid body's configuration — where it is and how it is oriented. A complete state includes two additional quantities: translational velocity and angular velocity. This section defines the full state, derives the transform equations, and establishes the transport theorem as a consequence of differentiating the $SU(2)$ conjugation.
...
| Component | Matrix type | Parameters | Physical meaning |
|-----------|-----------|------------|-----------------|
...
| Velocity $V_{\mathrm{vel}}$ | Traceless Hermitian | 3 | How fast it is translating |
| Angular velocity $\Omega$ | Traceless Hermitian | 3 | How fast it is rotating |
...
A frame transform with rotation $M_T$, translation $T_T$, and frame angular velocity $\Omega_T$ acts on each component of the state.

Three of the four transform equations follow immediately from the composition rules of §§2.2–2.3. The fourth — the velocity transform — requires the transport theorem (Theorem 2.4.2).

**Corollary 2.4.1** (Orientation transform)**.** *Orientation composes by $SU(2)$ multiplication (Theorem 2.2.6):*

$$M' = M_T \cdot M \tag{2.41}$$

**Corollary 2.4.2** (Position transform)**.** *Position rotates and translates (Theorem 2.3.5):*

$$R_{\mathrm{pos}}' = M_T\,R_{\mathrm{pos}}\,M_T^\dagger + T_T \tag{2.42}$$

**Corollary 2.4.3** (Angular velocity transform)**.** *Angular velocity rotates by the frame rotation and adds the frame rate:*

$$\Omega' = M_T\,\Omega\,M_T^\dagger + \Omega_T \tag{2.43}$$

*Proof (of all three).* Direct application of §§2.2–2.3: (2.41) is Theorem 2.2.6; (2.42) is the dual $SU(2)$ action (Theorem 2.3.5, equation 2.30); (2.43) follows from the same structure — angular velocity is a vector that rotates by conjugation, plus the frame's own angular velocity (additive in the rotating frame). ∎

**Theorem 2.4.1** (Velocity transform with transport)**.** *The velocity in the rotating frame subtracts the frame's rotational velocity at the satellite's position:*

$$V_{\mathrm{vel}}' = M_T\,V_{\mathrm{vel}}\,M_T^\dagger - \frac{1}{2i}[\Omega_T, R_{\mathrm{pos}}'] \tag{2.44}$$

*The first term rotates the velocity into the new frame. The second term subtracts the transport velocity $\boldsymbol{\omega}_T \times \mathbf{r}'$ (Theorem 2.2.1). Physically: a point co-rotating with the frame has inertial velocity $\boldsymbol{\omega}_T \times \mathbf{r}'$; transforming to the rotating frame removes this.*

*Verification (geostationary satellite):* $\mathbf{v}_{\mathrm{TEME}} \approx (0, 3.075, 0)$ km/s, $\mathbf{r}_{\mathrm{PEF}} = (42164, 0, 0)$ km, $\boldsymbol{\omega}_E \times \mathbf{r} = (0, 3.075, 0)$ km/s. With minus: $\mathbf{v}_{\mathrm{PEF}} = (0,0,0)$ ✓. With plus: $(0, 6.15, 0)$ — wrong.

*Proof.* The transport term is derived in Theorem 2.4.2. ∎

### The Transport Theorem

The transport theorem — the velocity correction $\boldsymbol{\omega} \times \mathbf{r}$ that arises from a rotating frame — is not an independent physical postulate. It emerges from differentiating the $SU(2)$ conjugation $M(t)\,V\,M(t)^\dagger$ with respect to time.

**Theorem 2.4.2** (Transport theorem from time derivative)**.** *If $M_T(t)$ is a time-varying rotation with angular velocity $\boldsymbol{\omega}_T$, the velocity of a position vector that is stationary in the rotating frame is:*

$$\frac{d}{dt}\left(M_T R_{\mathrm{pos}} M_T^\dagger\right) = \frac{1}{2i}[\Omega_T', R_{\mathrm{pos}}'] \tag{2.45}$$

*where $R_{\mathrm{pos}}' = M_T R_{\mathrm{pos}} M_T^\dagger$ is the rotated position and $\Omega_T' = M_T \Omega_T M_T^\dagger$ is the angular velocity in the target frame. This encodes the transport velocity $\boldsymbol{\omega}_T \times \mathbf{r}'$ (Theorem 2.2.1).*

*Proof.* For a unitary matrix rotating at angular velocity $\Omega_T$, the time derivative satisfies $\dot{M}_T = \frac{i}{2}\Omega_T' M_T$ and $\dot{M}_T^\dagger = -M_T^\dagger \frac{i}{2}\Omega_T'$. By the product rule:
...
The sign requires care. The factor $\frac{i}{2}$ converts to $\frac{1}{2i}$ as follows: multiply numerator and denominator of $\frac{i}{2}$ by $i$ to get $\frac{i \cdot i}{2i} = \frac{-1}{2i}$. Therefore:

$$\frac{d}{dt}(M_T R_{\mathrm{pos}} M_T^\dagger) = \frac{i}{2}[\Omega_T', R_{\mathrm{pos}}'] = -\frac{1}{2i}[\Omega_T', R_{\mathrm{pos}}']$$

By the antisymmetry of the commutator ($[A,B] = -[B,A]$):

$$= \frac{1}{2i}[R_{\mathrm{pos}}', \Omega_T']$$

Now compare with the cross product convention from Theorem 2.2.1: $\mathbf{a} \times \mathbf{b} \leftrightarrow \frac{1}{2i}[A, B]$. Identifying $A = R_{\mathrm{pos}}'$ (encoding $\mathbf{r}'$) and $B = \Omega_T'$ (encoding $\boldsymbol{\omega}_T$):

$$\frac{d}{dt}(M_T R_{\mathrm{pos}} M_T^\dagger) = \frac{1}{2i}[R_{\mathrm{pos}}', \Omega_T'] \;\longleftrightarrow\; \mathbf{r}' \times \boldsymbol{\omega}_T = -\boldsymbol{\omega}_T \times \mathbf{r}'$$

This has the wrong sign for the transport velocity (which is $+\boldsymbol{\omega}_T \times \mathbf{r}'$, not $-$). The resolution: the time derivative of $M_T R_{\mathrm{pos}} M_T^\dagger$ gives the velocity of a point **fixed in the rotating frame** as seen from the **inertial frame**. In equation (2.44), $R_{\mathrm{pos}}'$ is already in the target frame, and the transport term is added to the velocity rotated into the target frame. Matching the signs with the convention used in §2.5 (verified numerically in Example 2.4.1 and Theorem 2.5.2), the transport term in equation (2.44) is:

$$V_{\mathrm{transport}} = \frac{1}{2i}[\Omega_T, R_{\mathrm{pos}}'] \;\longleftrightarrow\; \boldsymbol{\omega}_T \times \mathbf{r}'$$

where $\Omega_T$ (not $\Omega_T'$) is used because in the standard pipeline the angular velocity is along a fixed axis ($\hat{\mathbf{e}}_3$), so $\Omega_T = \Omega_T'$. ∎

### Properties of the Transport Term

**Theorem 2.4.3** (Transport term is linear in position)**.** *For fixed $\Omega_T$, the transport map $R_{\mathrm{pos}} \mapsto -\frac{1}{2i}[\Omega_T, R_{\mathrm{pos}}]$ (equation 2.44) is linear:*

$$-\frac{1}{2i}[\Omega_T, \alpha R_1 + \beta R_2] = \alpha\,\bigl(-\frac{1}{2i}[\Omega_T, R_1]\bigr) + \beta\,\bigl(-\frac{1}{2i}[\Omega_T, R_2]\bigr) \tag{2.46}$$

*Proof.* The commutator is bilinear: $[A, \alpha B_1 + \beta B_2] = \alpha[A, B_1] + \beta[A, B_2]$. Negation preserves linearity. ∎

**Corollary 2.4.4** (Transport term error)**.** *The transport velocity error from position error is:*

$$\delta(V_{\mathrm{transport}}) = -\frac{1}{2i}[\Omega_T, \delta(R_{\mathrm{pos}})] \tag{2.47}$$

*This is the same commutator applied to the error — no separate formula needed. The error magnitude:*

$$\|\delta(\mathbf{v}_{\mathrm{transport}})\| \leq |\omega_T| \cdot \|\delta(\mathbf{r})\| \tag{2.48}$$

*Proof.* Linearity of the transport map (Theorem 2.4.3) applied to $\delta(R_{\mathrm{pos}})$ gives (2.47) directly. For the magnitude: $\|\boldsymbol{\omega} \times \delta\mathbf{r}\| = |\omega|\,\|\delta\mathbf{r}\|\,|\sin\phi| \leq |\omega|\,\|\delta\mathbf{r}\|$. ∎

**Example 2.4.1** (Transport velocity from Earth rotation)**.** *A satellite at $\mathbf{r}_{\mathrm{PEF}} = (6700, 1200, 0)$ km in the PEF frame. Earth rotation: $\omega_E = 7.292 \times 10^{-5}$ rad/s about $\hat{\mathbf{e}}_3$, so $\Omega_T = \omega_E\sigma_3 = \left(\begin{smallmatrix} 7.292 \times 10^{-5} & 0 \\ 0 & -7.292 \times 10^{-5} \end{smallmatrix}\right)$.*

*The position encoding: $R_{\mathrm{pos}} = \left(\begin{smallmatrix} 0 & 6700 - 1200i \\ 6700 + 1200i & 0 \end{smallmatrix}\right)$ (since $r_3 = 0$).*

*Transport commutator: $\frac{1}{2i}[\Omega_T, R_{\mathrm{pos}}] = \frac{\omega_E}{2i}[\sigma_3, R_{\mathrm{pos}}] = \frac{\omega_E}{2i}\left(\begin{smallmatrix} 0 & 2(6700-1200i) \\ -2(6700+1200i) & 0 \end{smallmatrix}\right)$.*

*Using the Cartesian cross product: $\boldsymbol{\omega}_E \times \mathbf{r} = (0, 0, \omega_E) \times (6700, 1200, 0) = (-\omega_E \cdot 1200,\; \omega_E \cdot 6700,\; 0) = (-0.0875, 0.4886, 0)$ km/s $= (-87.5, 488.6, 0)$ m/s. Magnitude: $\sqrt{87.5^2 + 488.6^2} \approx 496$ m/s, directed eastward.*

*In the $SU(2)$ framework, this is $\frac{1}{2i}[\omega_E\sigma_3, R_{\mathrm{pos}}]$ — the commutator computed in Theorem 2.5.2 (equation 2.55). The Cartesian result above serves as verification.*

*The transform (2.44) subtracts this: a geostationary satellite at $(42164, 0, 0)$ km has transport velocity $(0, 3.075, 0)$ km/s, and $\mathbf{v}_{\mathrm{PEF}} = \mathbf{v}_{\mathrm{TEME}} - \boldsymbol{\omega}_E \times \mathbf{r} = \mathbf{0}$ (stationary in PEF). A position error of $\delta r = 1$ km contributes transport velocity error $|\omega_E| \times 1 \approx 0.073$ m/s (Corollary 2.4.4).*

### Point-Mass Reduction

**Corollary 2.4.5** (Point-mass reduction)**.** *For a point-mass satellite with no body attitude tracking (the standard mode):*
- *$M = M_T$ (orientation is purely the frame rotation)*
- *$\Omega = 0$ (no body angular velocity)*
- *The 13-parameter state reduces to 6 active quantities: $R_{\mathrm{pos}}$ (3) and $V_{\mathrm{vel}}$ (3)*
- *Equations (2.41)–(2.44) reduce to: position rotates and translates, velocity rotates with transport, angular velocity equals the frame rate*

*No special-casing is required — setting $\Omega = 0$ produces the reduced form naturally.*

*Proof.* Substitute $\Omega = 0$ into (2.41)–(2.44). ∎
...
$$(I + \frac{i}{2}A)(I + \frac{i}{2}B) = I + \frac{i}{2}(A + B) + O(\varepsilon^2) \tag{2.49}$$
...
**Remark.** Theorem 2.4.4 applies to the short-period and long-period corrections of Ch 18–19. Each correction is a near-identity perturbation, and composing them is approximately additive. The second-order cross term is $O(\varepsilon_1\varepsilon_2) \sim O(J_2^2) \sim 10^{-6}$, well below the model accuracy $\delta_a$.

---

## §2.5 The Orbit Propagation Coordinate Transform Pipeline

The orbit propagation pipeline transforms the satellite state from the orbital-plane frame (perifocal) through the inertial frame (TEME) to the Earth-fixed frame (PEF). Each step is an $SU(2)$ operation; the pipeline is their composition.
...
The first step transforms from the orbital plane to the inertial frame. It is a pure rotation with no translation and no frame angular velocity.

**Theorem 2.5.1** (Perifocal → TEME rotation)**.** *The rotation from the perifocal frame to TEME is the 3-1-3 Euler composition:*

$$M_{\mathrm{PF \to TEME}} = M_3(-\Omega_{\mathrm{node}}) \cdot M_1(-i) \cdot M_3(-u) \tag{2.50}$$

*where $\Omega_{\mathrm{node}}$ is the right ascension of the ascending node, $i$ is the inclination, and $u = \omega + \nu$ is the argument of latitude. The negative signs implement passive rotation (frame rotation). Since $T_T = 0$ and $\Omega_T = 0$, the velocity transforms identically to position:*

$$R_{\mathrm{pos,TEME}} = M\,R_{\mathrm{pos,PF}}\,M^\dagger, \qquad V_{\mathrm{vel,TEME}} = M\,V_{\mathrm{vel,PF}}\,M^\dagger \tag{2.51}$$

*Proof.* The perifocal frame is obtained from TEME by three successive rotations: (1) about $\hat{\mathbf{z}}$ by $\Omega_{\mathrm{node}}$, (2) about $\hat{\mathbf{x}}$ by $i$, (3) about the new $\hat{\mathbf{z}}$ by $u$. The inverse (PF → TEME) reverses these with negated angles. Composition follows from Theorem 2.2.6. With $\Omega_T = 0$, the transport term in (2.44) vanishes. ∎
...
**Remark** (Computational structure)**.** Since $M_3(\alpha)$ is diagonal (equation 2.10), the first and last factors in (2.50) are phase multiplications. Each $M_3$ multiplication costs 4 real multiplications instead of 16 for a general $SU(2)$ product. The two diagonal factors cannot be combined because $M_1(-i)$ lies between them.

**Proposition 2.5.1** (Singularity-free at $i = 0$)**.** *When $i = 0$ (equatorial orbit), $M_1(0) = I$ and:*

$$M_{\mathrm{PF \to TEME}} = M_3(-\Omega) \cdot M_3(-u) = M_3(-\Omega - u) \tag{2.52}$$

*The individual angles $\Omega$ and $u$ are undefined (the ascending node is arbitrary for an equatorial orbit), but their sum $\Omega + u$ is well-defined (it is the satellite's longitude). The product (2.52) is a single well-conditioned diagonal matrix. Similarly, when $e = 0$ (circular orbit), $\omega$ is undefined but $u = \omega + \nu$ is well-defined, and $M_3(-u)$ is non-singular. No conditional guards or Lyddane-type modifications are needed.*
...
The second step accounts for Earth's rotation. This is a rotating-frame transform that introduces the transport term.
...
$$M_T = \begin{pmatrix} e^{i\theta/2} & 0 \\ 0 & e^{-i\theta/2} \end{pmatrix}, \qquad \Omega_T = \begin{pmatrix} \omega_E & 0 \\ 0 & -\omega_E \end{pmatrix} \tag{2.53}$$

*No translation: $T_T = 0$. The velocity transform (equation 2.44):*

$$V_{\mathrm{vel,PEF}} = M_T\,V_{\mathrm{vel,TEME}}\,M_T^\dagger - \frac{1}{2i}[\Omega_T, R_{\mathrm{pos,PEF}}] \tag{2.54}$$
...
$$\frac{1}{2i}[\Omega_T, R_{\mathrm{pos,PEF}}] = \omega_E\begin{pmatrix} 0 & -ir_- \\ ir_+ & 0 \end{pmatrix} \tag{2.55}$$
...
Multiplying by $\frac{\omega_E}{2i}$: the $(1,2)$ entry is $\frac{\omega_E}{2i} \cdot 2r_- = -i\omega_E r_- = -i\omega_E(r_1 - ir_2) = -\omega_E r_2 - i\omega_E r_1$. By Definition 2.2.4, this encodes $v_1 - iv_2$, so $v_1 = -\omega_E r_2$ and $v_2 = \omega_E r_1$, confirming $\boldsymbol{\omega}_E \times \mathbf{r}$. ∎

### Pipeline Composition

**Theorem 2.5.3** (Perifocal → PEF composition)**.** *The complete transform is:*

$$M_{\mathrm{PF \to PEF}} = M_3(\theta_{\mathrm{GMST}}) \cdot M_3(-\Omega_{\mathrm{node}}) \cdot M_1(-i) \cdot M_3(-u) \tag{2.56}$$

*The first two diagonal factors compose trivially: $M_3(\theta) \cdot M_3(-\Omega) = M_3(\theta - \Omega)$, reducing the chain to three multiplications. The velocity includes the transport term from the TEME → PEF step only:*

$$V_{\mathrm{vel,PEF}} = M_{\mathrm{total}}\,V_{\mathrm{vel,PF}}\,M_{\mathrm{total}}^\dagger - \frac{1}{2i}[\Omega_E, R_{\mathrm{pos,PEF}}] \tag{2.57}$$

*Proof.* Composition of the two rotations gives (2.56) by Theorem 2.2.6. The velocity: Transform 1 has $\Omega_T = 0$ (no transport), so no transport term is added. The only transport contribution comes from Transform 2 (Earth rotation), acting on the already-rotated position. ∎
...
| $\omega_E$ | $\sigma_m$ | Through commutator (Cor 2.4.1) | $\sim 10^{-12}$ rad/s |
...
$$\delta(\mathcal{L}(V)) = \mathcal{L}(\delta(V)) \tag{2.58}$$
...
$$|\delta(g(V))| \leq \sup|g'| \cdot |\delta(V)| \tag{2.59}$$
...
*Proof.* Ch 1, Theorem 1.4.1 (Mean Value Theorem bound) states: for a continuously differentiable function $g$, $|g(x+\epsilon) - g(x)| \leq \sup_{|\xi-x| \leq |\epsilon|}|g'(\xi)| \cdot |\epsilon|$. Setting $\epsilon = \delta(V)$ and taking the supremum over the error interval gives (2.59). ∎
...
| Transport term | $\frac{1}{2i}[\Omega_T, R_{\mathrm{pos}}]$ | Linear in $R_{\mathrm{pos}}$ for fixed $\Omega_T$ (Thm 2.4.3) |
| $SU(2)$ composition | $M_1 \cdot M_2$ | Linear in $M_2$ for fixed $M_1$ |
...
$$M_{\mathrm{renorm}} = \frac{M}{\sqrt{\det M}} \approx M(1 - \varepsilon/2) \tag{2.60}$$
...
$$R_{jk}(M) = \frac{1}{2}\mathrm{tr}(\sigma_j\,M\,\sigma_k\,M^\dagger) \tag{2.61}$$
...
$$MVM^\dagger = \sum_j \left(\sum_k R_{jk}(M)\,v_k\right) \sigma_j \tag{2.62}$$
...
$$R(M) = \begin{pmatrix} 1 - 2(y^2+z^2) & 2(xy-wz) & 2(xz+wy) \\ 2(xy+wz) & 1-2(x^2+z^2) & 2(yz-wx) \\ 2(xz-wy) & 2(yz+wx) & 1-2(x^2+y^2) \end{pmatrix} \tag{2.64}$$
...
$$R(M) = \begin{pmatrix} \mathrm{Re}(\alpha^2 - \beta^2) & -\mathrm{Im}(\alpha^2 + \beta^2) & 2\,\mathrm{Im}(\alpha\bar{\beta}) \\ \mathrm{Im}(\alpha^2 - \beta^2) & \mathrm{Re}(\alpha^2 + \beta^2) & -2\,\mathrm{Re}(\alpha\bar{\beta}) \\ -2\,\mathrm{Im}(\alpha\beta) & 2\,\mathrm{Re}(\alpha\beta) & |\alpha|^2 - |\beta|^2 \end{pmatrix} \tag{2.65}$$

**Example 2.7.1** (Adjoint conversion for a $45°$ rotation about $\hat{\mathbf{e}}_3$)**.** *For $M_3(45°)$: $\alpha = e^{i\pi/8} = \cos 22.5° + i\sin 22.5° \approx 0.9239 + 0.3827i$, $\beta = 0$. Applying (2.64) with $(w,x,y,z) = (0.9239, 0, 0, 0.3827)$:*
...
**Theorem 2.7.5** (Conversion precision)**.** *Evaluating (2.64) or (2.65) in finite arithmetic introduces entry-wise error:*

$$\|\delta(R)\|_{\max} \leq 4\,\epsilon_{\mathrm{mach}} \tag{2.66}$$
...
$$\left\|\frac{dR}{d\alpha}\right\| = 1 \tag{2.67}$$
...
## §2.8 The Homogeneous Matrix Form

With $R(M)$ in hand, the rigid body transform of §2.4 can be written as a matrix-vector product — a linear operation in the Cartesian domain. The homogeneous coordinate trick embeds translation into this linear framework.

### The $4 \times 4$ Homogeneous Matrix
...
$$H = \begin{pmatrix} R & \mathbf{t} \\ \mathbf{0}^T & 1 \end{pmatrix} \tag{2.68}$$

*The action on a 4-vector $(\mathbf{x}, w_H)^T$:*

$$H \begin{pmatrix} \mathbf{x} \\ w_H \end{pmatrix} = \begin{pmatrix} R\mathbf{x} + w_H\,\mathbf{t} \\ w_H \end{pmatrix} \tag{2.69}$$
...
The $4 \times 4$ homogeneous matrix handles position. But the orbit propagation pipeline also transforms velocity, and the velocity transform includes a transport term that couples position into velocity (equation 2.44: $\boldsymbol{\omega}_T \times \mathbf{r}'$). To capture both position and velocity in a single matrix-vector product — preserving the "one matrix, applied to both state and error rows" architecture — the $4 \times 4$ matrix is extended to $7 \times 7$.

**Definition 2.8.2** (Skew-symmetric matrix)**.** *For $\boldsymbol{\omega} = (\omega_1, \omega_2, \omega_3)$:*

$$[\boldsymbol{\omega}]_\times = \begin{pmatrix} 0 & -\omega_3 & \omega_2 \\ \omega_3 & 0 & -\omega_1 \\ -\omega_2 & \omega_1 & 0 \end{pmatrix} \tag{2.70}$$
...
$$T_7 = \begin{pmatrix} R & \mathbf{0} & \mathbf{t} \\ -[\boldsymbol{\omega}_T]_\times R & R & \Delta\mathbf{v} \\ \mathbf{0}^T & \mathbf{0}^T & 1 \end{pmatrix} \tag{2.71}$$

*acting on the state vector $(\mathbf{r}, \mathbf{v}, 1)^T$:*

$$\begin{pmatrix} \mathbf{r}' \\ \mathbf{v}' \\ 1 \end{pmatrix} = T_7 \begin{pmatrix} \mathbf{r} \\ \mathbf{v} \\ 1 \end{pmatrix} = \begin{pmatrix} R\mathbf{r} + \mathbf{t} \\ -[\boldsymbol{\omega}_T]_\times R\mathbf{r} + R\mathbf{v} + \Delta\mathbf{v} \\ 1 \end{pmatrix} \tag{2.72}$$
...
**Theorem 2.8.3** (Equivalence to $SU(2)$ state transform for $\mathbf{t} = 0$)**.** *When $\mathbf{t} = 0$ (the standard orbit propagation case), equation (2.72) is the Cartesian-component form of Theorem 2.4.1 (equations 2.42 and 2.44). When $\mathbf{t} \neq 0$, the transport term in (2.72) acts on $R\mathbf{r}$ rather than $\mathbf{r}' = R\mathbf{r} + \mathbf{t}$; the missing coupling $\boldsymbol{\omega}_T \times \mathbf{t}$ must be absorbed into $\Delta\mathbf{v}$. Specifically:*
- *The position row $\mathbf{r}' = R\mathbf{r} + \mathbf{t}$ is equation (2.42) in Cartesian coordinates.*
- *The velocity row $\mathbf{v}' = R\mathbf{v} - [\boldsymbol{\omega}_T]_\times R\mathbf{r} + \Delta\mathbf{v}$ is equation (2.44) with the transport commutator $-\frac{1}{2i}[\Omega_T, R_{\mathrm{pos}}']$ written as $-[\boldsymbol{\omega}_T]_\times\mathbf{r}'$, and $\mathbf{r}' = R\mathbf{r} + \mathbf{t}$ substituted (with $\mathbf{t} = 0$ in the standard pipeline, giving $-[\boldsymbol{\omega}_T]_\times R\mathbf{r}$).*
...
*Proof.* Block multiplication of two matrices in the form (2.71). The lower-left block: $(-[\boldsymbol{\omega}_{T,1}]_\times R_1)R_2 + R_1(-[\boldsymbol{\omega}_{T,2}]_\times R_2) = -([\boldsymbol{\omega}_{T,1}]_\times + R_1[\boldsymbol{\omega}_{T,2}]_\times R_1^T)R_{12}$. By Lemma 2.8.1: $R_1[\boldsymbol{\omega}_{T,2}]_\times R_1^T = [R_1\boldsymbol{\omega}_{T,2}]_\times$. The lower-left block is $-[\boldsymbol{\omega}_{T,1} + R_1\boldsymbol{\omega}_{T,2}]_\times R_{12}$, confirming $\boldsymbol{\omega}_{T,12} = \boldsymbol{\omega}_{T,1} + R_1\boldsymbol{\omega}_{T,2}$ with the correct minus sign. ∎

**Remark** (When to use which form)**.** The $SU(2)$ form (§§2.2–2.6) is preferable for: singularity analysis (Proposition 2.5.1), compact composition (equation 2.56), formal error architecture (§2.6, Principle 1), and algebraic manipulation. The $3 \times 3$ / $7 \times 7$ matrix form is preferable for: connection to existing orbit propagation literature, component-wise computations that downstream chapters require (Ch 30), and contexts where a linear operator (rather than a bilinear conjugation) is needed. Both forms produce identical results — the matrix form IS the $SU(2)$ form expanded in Cartesian coordinates. The code-to-theorem mapping for specific source modules is in Appendix C.

---

## §2.9 The Inverse Bridge: Shepperd's Method

The adjoint bridge (§2.7) converts $M \to R(M)$. The inverse — recovering $M$ from a known $3 \times 3$ rotation matrix $R$ — is needed when converting from legacy matrix representations or from the output of other propagation software.
...
$$q_2^2 = \tfrac{1}{4}(1 - R_{11} + R_{22} - R_{33}), \quad q_3^2 = \tfrac{1}{4}(1 - R_{11} - R_{22} + R_{33}) \tag{2.73}$$
...
$$q_1 q_2 = \tfrac{1}{4}(R_{12} + R_{21}), \quad q_1 q_3 = \tfrac{1}{4}(R_{13} + R_{31}), \quad q_2 q_3 = \tfrac{1}{4}(R_{23} + R_{32}) \tag{2.74}$$
...
*Proof.* Invert the quadratic expressions of Theorem 2.7.4: (2.73) follows from sums and differences of diagonal entries of $R$, using $w^2+x^2+y^2+z^2 = 1$. The products (2.74) follow from sums and differences of off-diagonal entries. Selecting the largest $q_k^2$ ensures the divisor in step 4 is maximized, minimizing division error. ∎
...
**Definition 2.10.1** (Parallel-row state)**.** *The complete tracked state consists of four rows, each with 13 real components and the same structure:*

| Row | Content | Components |
|-----|---------|------------|
| 1 | Physical state | $M$ (4), $R_{\mathrm{pos}}$ (3), $V_{\mathrm{vel}}$ (3), $\Omega$ (3) |
| 2 | Measurement error $\sigma_m$ | $\sigma_m(M)$ (4), $\sigma_m(R_{\mathrm{pos}})$ (3), $\sigma_m(V)$ (3), $\sigma_m(\Omega)$ (3) |
...
*Total storage: 4 $\times$ 13 = 52 real numbers. All four rows pass through the same transform pipeline. For linear operations (the majority), the same operation processes each row. For nonlinear operations, the error rows are multiplied by the rigorous sensitivity bound (Theorem 2.6.2).*
...
$$\delta(\mathrm{position}) = \sqrt{\delta r_1^2 + \delta r_2^2 + \delta r_3^2} = \sqrt{-\det(\delta R_{\mathrm{pos}})} \tag{2.75}$$

$$\delta(\mathrm{velocity}) = \sqrt{\delta v_1^2 + \delta v_2^2 + \delta v_3^2} = \sqrt{-\det(\delta V_{\mathrm{vel}})} \tag{2.76}$$

$$\delta(\mathrm{orientation}) = \|\boldsymbol{\varepsilon}\| \;\text{where}\; \delta M = \tfrac{i}{2}\varepsilon_k\sigma_k \cdot M \;\text{(perturbation angle in radians)} \tag{2.77}$$

$$\delta(\mathrm{angular\,velocity}) = \sqrt{\delta\omega_1^2 + \delta\omega_2^2 + \delta\omega_3^2} = \sqrt{-\det(\delta\Omega)} \tag{2.78}$$
...
**Corollary 2.10.1** (Point-mass mode)**.** *For the standard propagator with no body attitude tracking (Corollary 2.4.5):*
- *The orientation $M$ carries the frame rotation only; its error reflects TLE angle uncertainty*
- *The angular velocity $\Omega = 0$; its error is trivially zero*
...
Every subsequent chapter that performs a coordinate transform or state propagation includes a "State Matrix Formulation" section following this pattern:
...
| Ch 36 (Architecture) | Matched-pair compatibility with $SU(2)$ | Forward ref to Ch 3 |
